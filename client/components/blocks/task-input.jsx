...
const TaskInput = ({ task, setTask, setTaskSubmitting, taskSubmitting }) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e) =>
      inputCallbacks.handleGlobalKeyDown(e, textareaRef);
  ...
  useEffect(() => {
    setTask('post "hello" on my twitter account');
  }, []);

  return (
    <div
      className="border border-neutral-200 rounded-4xl p-6 bg-white w-[800px] cursor-text"
    ...
    >
      <textarea
        ...
        rows={rows}
        onChange={(e) =>
          inputCallbacks.handleTextareaChange(e, setRows, setTask)
        }
        onKeyDown={(e) =>
          inputCallbacks.handleKeyDown(e, task, setTaskSubmitting)
        }
        value={task}
      ...
      <div className="flex items-end justify-between">
        ...
        <button
          ...
          onClick={() => {
            task.trim() !== "" ? setTaskSubmitting(true) : null;
          }}
          disabled={taskSubmitting}
        >
          {taskSubmitting ? (
            <Spinner className="size-6" color="white" />
          ) : (

...
export const TaskValidationSchema = z.object({
  isTaskValid: z.boolean().describe('Whether the task is valid and safe to process'),
 ...
 */
export async function validateTask(task: TaskData): Promise<TaskValidationResult> {
  try {
    const OPENAI_API_KEY = process.env.OPENAI_API_KEY;
    
    if (!OPENAI_API_KEY) {
      return { isTaskValid: true, reason: 'OpenAI API key not set, validation skipped' };
    }
    ...
    const { object } = await generateObject({
      ...
      schemaDescription: 'Validation result for a task to determine if it is safe, appropriate, and within the capabilities of Orchestrator',
      system: 
